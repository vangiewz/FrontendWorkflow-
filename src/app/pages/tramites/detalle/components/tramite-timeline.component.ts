    ...
    <div class="relative">
      @for (paso of pasosOrdenados; track paso.id; let i = $index) {
        ...
        <div class="flex gap-3 mb-1" 
             [class.cursor-pointer]="isPasoCompletado(paso.id) || paso.id === actualStepId" 
          ...
          <div class="flex flex-col items-center">
            <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 border-2 transition-all"
              ...
              [class.shadow-blue-500/50]="selectedStepId === paso.id && paso.id === actualStepId && !isPasoCompletado(paso.id)"
              [class.text-white]="isPasoCompletado(paso.id) || paso.id === actualStepId"
              [class.bg-blue-500]="paso.id === actualStepId && !isPasoCompletado(paso.id)"
              [class.border-blue-500]="paso.id === actualStepId && !isPasoCompletado(paso.id)"
              [class.bg-surface-800]="!isPasoCompletado(paso.id) && paso.id !== actualStepId"
              [class.border-surface-600]="!isPasoCompletado(paso.id) && paso.id !== actualStepId"
              [class.text-gray-500]="!isPasoCompletado(paso.id) && paso.id !== actualStepId"
              [class.animate-pulse]="paso.id === actualStepId && !isPasoCompletado(paso.id)"
            >
          ...
          <div class="pb-4 flex-1 min-w-0 flex items-center justify-between group">
            <div>
               <p class="text-sm font-medium truncate"
                 [class.text-white]="paso.id === actualStepId || isPasoCompletado(paso.id)"
                 [class.text-gray-500]="!isPasoCompletado(paso.id) && paso.id !== actualStepId"
               >
                 {{ paso.nombrePaso }}
               </p>
            ...
            @if (isPasoCompletado(paso.id) || paso.id === actualStepId) {
...
export class TramiteTimelineComponent {
  ...
  @Input() actualStepId: string | null | undefined = null;
  ...
  onStepClick(pasoId: string) {
    if (this.isPasoCompletado(pasoId) || pasoId === this.actualStepId) {
